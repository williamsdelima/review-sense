(() => {
  const API_BASE_URL = "https://review-sense-production.up.railway.app/api/v1";
  ...
  const renderResult = (data) => {
    ...
    takeaways.replaceChildren(...(data.key_takeaways || []).map((item) => {
      const entry = document.createElement("li"); entry.textContent = item; return entry;
    }));
    result.hidden = false;
    languageButtons.forEach((item) => item.classList.toggle("is-active", item.dataset.language === currentLanguage));
  ...
  const request = async (path, body) => {
    ...
    if (!response.ok) throw new Error(data.detail || "Não foi possível processar este link. Tente novamente.");
    return data;
  };
  form.addEventListener("submit", async (event) => {
    ...
    setLoading(true);
    try { currentLanguage = "en"; renderResult(await request("/summarize", { url: input.value.trim() })); }
  ...
  languageButtons.forEach((languageButton) => languageButton.addEventListener("click", async () => {
    ...
    try { currentLanguage = targetLanguage; renderResult(await request("/translate", { url: input.value.trim(), target_language: targetLanguage })); }
    catch (err) { currentLanguage = "en"; showError(err.message); }
    finally { setLoading(false); }
  }));
})();
